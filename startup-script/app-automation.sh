...
sudo aws s3 cp s3://three-tier-web-application-configurations-975050296235/apache-config/000-default.conf /etc/apache2/sites-enabled/
...
sudo aws s3 cp s3://three-tier-web-application-configurations-975050296235/database-config/wp-config.php /var/www/wordpress/
